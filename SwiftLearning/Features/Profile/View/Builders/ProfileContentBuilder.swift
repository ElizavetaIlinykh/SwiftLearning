...
struct ProfileContentBuilder {
    ...
    func build(content: ProfileContent) -> ProfileContentViewData {
        ProfileContentViewData(
            header: ProfileHeaderViewData(
                name: content.user.name,
                email: content.user.email
            ),
            progress: progressViewData(statistics: content.statistics),
    ...
    private func progressViewData(statistics: UserStatistics) -> ProfileProgressViewData {
        ProfileProgressViewData(
    ...
    private func statisticCards(statistics: UserStatistics) -> [StatCardViewData] {
        [
            StatCardViewData(title: L10n.string("profile.stat.level"), value: "\(statistics.currentLevel)", systemImage: "bolt.fill"),
            StatCardViewData(
                title: L10n.string("profile.stat.lessons"),
                value: "\(statistics.completedLessonsCount) / \(statistics.totalLessonsCount)",
                systemImage: "book.fill"
            ),
            StatCardViewData(title: L10n.string("profile.stat.progress"), value: "\(statistics.progressPercent)%", systemImage: "target")
        ]
    }

    private func achievements(statistics: UserStatistics) -> [AchievementCardViewData] {
